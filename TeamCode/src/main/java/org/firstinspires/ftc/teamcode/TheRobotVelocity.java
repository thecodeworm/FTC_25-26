...
@TeleOp(name="TheRobot (Velocity)", group="Competition")
public class TheRobotVelocity extends LinearOpMode {
    ...
    /* ==================== SHOOTER CONSTANTS ==================== */
    // Yellow Jacket 312 RPM (19.2:1) - VERIFY THIS MATCHES YOUR MOTOR!
    private static final double TICKS_PER_REV = 537.6;
    private static final double MAX_TICKS_PER_SEC = 2800;

    // Idle RPM - keep shooter spinning when enabled
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            // Toggle shooter enable with DPad Down
            ...
            // Maintain idle velocity when enabled but not shooting
            if (shooterEnabled && !isShooting) {
    ...
    private void autoAim() {
        ...
        double targetYawAngle = calculateYawAngle(distance);
    ...
    private void executeShootSequence() {
        ...
        // Wait for spin-up (motor PID is handling velocity)
        sleep(600);
     ...
     *
     * TUNING GUIDE:
     * - Test at 24", 48", 72", 96", 120"
     * - If shots fall short: Increase base (220 → 240) or multiplier (1.8 → 2.0)
     * - If shots arc too high: Decrease base or multiplier
     *
     * Current formula: RPM = 220 + (distance * 1.8)
     * 24" → 263 RPM
     * 48" → 306 RPM
     * 72" → 350 RPM
     * 96" → 393 RPM
