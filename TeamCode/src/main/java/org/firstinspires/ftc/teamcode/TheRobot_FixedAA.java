// Version 6 - Aditya Chanda - OPTIMIZED WITH VELOCITY CONTROL
...
@TeleOp(name="TheRobot_OptimizedVelocity", group="Competition")
public class TheRobot_FixedAA extends LinearOpMode {
    ...
    private static final double TICKS_PER_REV = 1680.0;
    private static final double MAX_RPM = 6000.0;
    private static final double MAX_TICKS_PER_SECOND = (MAX_RPM / 60.0) * TICKS_PER_REV; // 168,000 tps

    // Optimized PIDF coefficients for fast response
    private static final double SHOOTER_kP = 50.0;
    private static final double SHOOTER_kI = 0.5;
    private static final double SHOOTER_kD = 6.0;
    ...
    private static final double IDLE_SHOOTER_VELOCITY = 120000;  // ~4300 RPM idle (pre-spin)
    ...
    private static final double BOOST_MULTIPLIER = 1.10;  // Overshoot by 10%
    ...
    private static final double SLOT_1_POSITION = 0.42;
    private static final double SLOT_2_POSITION = 0.82;

    // Shooting positions
    private static final double SHOOT_POSITION_0 = 0.61;
    private static final double SHOOT_POSITION_1 = 1.0;
    private static final double SHOOT_POSITION_2 = 0.2;

    // Kicker positions
    private static final double KICKER_OPEN = 0;
    private static final double KICKER_CLOSED = 0.37;
    ...
    private void autoAim() {
        ...
        // Calculate shooter velocity (not power)
        calculatedShooterVelocity = calculateShooterVelocity(filteredDist);
    ...
    /**
     * Calculate shooter velocity in ticks per second based on distance
     * Converts your original power calculation to velocity
     */
    private double calculateShooterVelocity(double distanceInches) {
        // Your original power formula
        double power = 0.52 + ((Math.pow(distanceInches, distanceInches * 0.004)) / 100);
        power = Math.max(0.50, Math.min(0.85, power));

        // Convert power to velocity (approximate)
        // At full power (1.0), motor should reach ~168,000 tps
        double velocity = power * MAX_TICKS_PER_SECOND;

        return Math.max(80000, Math.min(MAX_TICKS_PER_SECOND * 0.95, velocity));
    }
    ...
    private void executeShootSequence() {
        ...
        // OPTIMIZED: Use velocity boost technique for fast spin-up
        ...
        // Wait for velocity stabilization with faster method
        ...
        telemetry.addData("Initial Velocity", "%.0f tps (%.0f RPM)", initialVelocity, (initialVelocity/TICKS_PER_REV)*60);
        ...
        for (int shootIndex = 0; shootIndex < 3; shootIndex++) {
            ...
            if (shootIndex > 0) {
                ...
                sleep(moveDelay + 100);  // Reduced delay
        ...
        telemetry.addData("Complete", "3 balls fired with optimized velocity control");
        telemetry.update();
        sleep(1000);
    }
